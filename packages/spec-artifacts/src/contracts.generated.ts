...
export type Ruleset = {
  /** Schema identifier used to validate this Artifact. */
  readonly "$schema": "urn:voidtrace:schema:ruleset:0.16.0";
  /** Stable discriminator for this Artifact kind. */
  readonly "kind": "ruleset";
  /** Version of this Artifact contract. */
  readonly "schemaVersion": "0.16.0";
  /** Stable identity of this Artifact. */
  readonly "id": string;
  /** Non-negative immutable revision of this Artifact. */
  readonly "revision": number;
  ...
  readonly "rules": ReadonlyArray<{
    ...
    readonly "phase": "attack.emit" | "damage.construct" | "critical.roll" | "critical.expected" | "critical.resolve" | "damage.radial-falloff" | "status.tick" | "target.mitigate" | "damage.commit" | "result.aggregate";
    ...
    readonly "operation": {
      ...
      /** Expand one resolved ricochet path into an explicit bounded count of ordered target events. */
      readonly "kind": "event.expand-resolved-ricochet-targets";
      /** Positive safe execution bound for one resolved ricochet path. */
      readonly "maximumTargets": number;
    } | {
      /** Expand one resolved chain path into an explicit bounded count of ordered target events. */
      readonly "kind": "event.expand-resolved-chain-targets";
      /** Positive safe execution bound for one resolved chain path. */
      readonly "maximumTargets": number;
    } | {
      /** Expand one resolved Radial impact relation set into an explicit bounded count of target events. */
      ...
      /** Construct one synthetic Status tick Damage Vector from explicit resolved Health damage. */
      readonly "kind": "damage-vector.copy-resolved-status-tick";
    } | {
      /** Resolve safe non-negative Critical tiers from an explicit roll and the fractional part of Critical chance; integer chance keeps next tier equal to base tier. */
      readonly "kind": "critical-tier.resolve-tier-roll";
    } | {
      /** Resolve the safe non-negative adjacent Critical tiers and their probabilities without selecting a realized tier. */
      readonly "kind": "critical-tier.resolve-expected-branches";
    } | {
      ...
      /** Aggregate terminal Damage and target-specific Health for a resolved ricochet path. */
      readonly "kind": "damage-vector.aggregate-resolved-ricochet-targets";
    } | {
      /** Aggregate terminal Damage and target-specific Health for a resolved chain path. */
      readonly "kind": "damage-vector.aggregate-resolved-chain-targets";
    } | {
