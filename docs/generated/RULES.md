...
| `rule.beam.schedule-resolved-ticks` | `attack.emit` | `event.expand-resolved-beam-ticks` | `experimental` | Expand one explicit resolved Beam action into a bounded count of ordered logical-time tick events. |
...
| `rule.radial.expand-resolved-targets` | `attack.emit` | `event.expand-resolved-radial-targets` | `experimental` | Expand one explicit resolved impact-distance relation set into bounded target Radial events. |
| `rule.impact.expand-resolved-direct-radial` | `attack.emit` | `event.expand-resolved-direct-radial-impact` | `experimental` | Expand one resolved impact into one ordered Direct child followed by bounded Radial target children. |
...
| `rule.beam.construct-resolved-tick` | `damage.construct` | `damage-vector.copy` | `experimental` | Construct one Beam tick Damage Vector from the resolved Beam attack mode. |
...
| `rule.beam.scale-critical-tier` | `critical.resolve` | `damage-vector.scale-critical-tier` | `experimental` | Scale one Beam tick by its explicit fixed Critical tier. |
...
| `rule.beam.standard-armor` | `target.mitigate` | `damage-vector.scale-standard-armor` | `experimental` | Apply the resolved target Armor multiplier to one Beam tick. |
...
| `rule.beam.commit-resolved-tick-health` | `damage.commit` | `damage.commit-health` | `experimental` | Commit one resolved Beam tick to the target Health layer. |
...
| `rule.beam.aggregate-resolved-ticks` | `result.aggregate` | `damage-vector.aggregate-sequential-beam-ticks` | `experimental` | Aggregate every terminal Beam tick Damage Vector and preserve the final sequential remaining Health. |
...
| `rule.radial.aggregate-resolved-targets` | `result.aggregate` | `damage-vector.aggregate-resolved-radial-targets` | `experimental` | Aggregate terminal Damage and target-specific Health from every target inspected by one resolved Radial impact. |
| `rule.impact.aggregate-resolved-direct-radial` | `result.aggregate` | `damage-vector.aggregate-resolved-direct-radial-impact` | `experimental` | Aggregate one terminal Direct Damage Vector, all terminal Radial Damage Vectors, and target-specific Health for one resolved impact. |
