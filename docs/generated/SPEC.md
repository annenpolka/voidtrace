...
- Source fingerprint: `sha256:ee6cc97c9382690c57e6d1ba3dd74be7f666f0006e757cef7ac2002841bec0e5`
...
| `SCP-003` | `scope` | `unsupported_mechanic_rejected` | `property-tested` | `active` | Scenario targetGraphは、Kernel外で解決済みのimpact距離・LoSまたはordered pathだけを有限構造として保持する。Runtimeは空relations、resolved punch-through／ricochet／chain Direct Hit列が参照する一つのordered path、またはresolved multi-target Radialが参照する同一impactの1以上64以下のimpact-distance relationsだけを受理し、その他の非空Target Graphや複数targetを黙って無視せずunsupportedとして部分Artifactなしで拒否する |
