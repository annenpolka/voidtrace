...
const experimentPath = resolve(
  repositoryRoot,
  "data/fixtures/experiments/patch-backed-scenario-comparison.experiment.json",
);
const expectedPath = resolve(
  repositoryRoot,
  "data/fixtures/experiments/patch-backed-scenario-comparison.expected.json",
);
...
const HELP = `VoidTrace repository-local Patch-backed comparison helper

Usage:
  node .agents/skills/voidtrace/scripts/run-patch-comparison.ts [options]
...
async function main(argv: readonly string[]): Promise<number> {
  ...
  const [experiment, expected, catalog, scenario, patch] = await Promise.all([
  ...
  if (
    ...
    throw new AdapterError(
      ...
      "Patch-backed comparison did not match literal expectations",
    );
  }
...
try {
  ...
  const adapterError =
    error instanceof AdapterError
      ? error
      : new AdapterError("adapter.internal", "Patch-backed comparison helper failed unexpectedly");
  process.stdout.write(
