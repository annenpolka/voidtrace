...
- Use only the finite Contract IR from `contracts/model.pkl`; do not embed JSON Schema or TypeScript fragments.
- Do not embed TypeScript, JavaScript, Rust, Python, or shell snippets in Pkl.
- Clause IDs are permanent and must remain unique.
- Contract IDs, type names, Schema IDs, and versions are public compatibility surfaces.
