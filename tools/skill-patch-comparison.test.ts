...
const helperPath = resolve(
  repositoryRoot,
  ".agents/skills/voidtrace/scripts/run-patch-comparison.ts",
);
...
describe("Patch-backed Experiment repository-local helper", () => {
  it("passes the checked-in literal comparison golden", () => {
    ...
    expect(parseOutput(result)).toMatchObject({
      ok: true,
      comparison: {
        id: "comparison.experiment.golden-patch-backed-comparison",
  ...
  it("shows help without loading fixtures", () => {
    ...
    expect(result.status).toBe(0);
    expect(result.stderr).toBe("");
    expect(result.stdout).toContain("VoidTrace repository-local Patch-backed comparison helper");
