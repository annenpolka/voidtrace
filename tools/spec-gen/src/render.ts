...
export function renderGeneratedFiles(spec: SpecDocument): GeneratedFile[] {
  ...
  const multishotClauses = spec.clauses.filter(
  ...
  const beamClauses = spec.clauses.filter(
    (clause) => clause.id.startsWith("BEM-") || clause.id === "GOL-018",
  );
  ...
  const chainClauses = spec.clauses.filter(
    (clause) => clause.id.startsWith("CHN-") || clause.id === "GOL-011",
  );
  ...
  const capabilities = {
    ...
    capabilities: [
      ...
      capability("mechanics.resolved-beam-ticks", beamClauses),
